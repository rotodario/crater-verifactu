...
class UpdateInstallationController extends Controller
{
    public function __invoke(Request $request)
    {
        ...
        $validated = $request->validate([
            ...
            // Software / SIF identification
            'vendor_name'         => ['nullable', 'string', 'max:120'],
            'vendor_tax_id'       => ['nullable', 'string', 'max:20'],
            'software_id'         => [
                function ($attr, $value, $fail) use ($request) {
                    $mode = $request->input('mode');
                    if (in_array($mode, ['aeat_sandbox', 'aeat_production']) && empty($value)) {
                        $fail('El IdSistemaInformatico es obligatorio en modo sandbox o producción.');
                    }
                },
