...
/**
 * Queries AEAT ConsultaFactuSistemaFacturacion for the remote historial
 * of registered invoices for a given period. Read-only.
 *
 * POST /api/v1/verifactu/historial
 */
class VerifactuHistorialController extends Controller
{
    public function __invoke(Request $request)
    {
        ...
        if (! $installation) {
            return response()->json(['success' => false, 'error' => 'No hay instalación VERI*FACTU configurada para esta empresa.'], 422);
        }

        $company = $installation->company;
        if (! $company || ! $company->tax_number) {
            return response()->json(['success' => false, 'error' => 'La empresa no tiene NIF configurado.'], 422);
        }
        ...
        if (! in_array($mode, ['aeat_sandbox', 'aeat_production'])) {
            return response()->json([
                'success' => false,
                'error'   => "El historial remoto solo está disponible en modos aeat_sandbox o aeat_production (modo actual: {$mode}).",
            ], 422);
        }

        $service = new AeatHistorialService();
