...
use Crater\Services\Verifactu\VerifactuQrService;
...
class Invoice extends Model implements HasMedia
{
    ...
    public const FISCAL_STATUS_NOT_ISSUED = 'NOT_ISSUED';
    public const FISCAL_STATUS_ISSUED = 'ISSUED';
    ...
    public function rectificativeInvoices()
    {
    ...
    public function isFiscalIssued()
    {
        return $this->fiscal_status !== self::FISCAL_STATUS_NOT_ISSUED
            && ! is_null($this->fiscal_issued_at);
    }

    public function isFiscalLocked()
    {
        return ! is_null($this->fiscal_locked_at);
    }
    ...
    {
        if (config('verifactu.issue_on_send', true)) {
            app('Crater\Services\Verifactu\VerifactuService')->ensureIssued($this, [
                'trigger' => 'invoice_send',
            ]);
        }
    ...
    public static function deleteInvoices($ids)
    {
        foreach ($ids as $id) {
            ...
            if ($invoice && $invoice->isFiscalLocked()) {
                return 'invoice_fiscally_locked';
            }
        }
