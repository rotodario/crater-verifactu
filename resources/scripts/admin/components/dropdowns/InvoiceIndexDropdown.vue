<template>
  <BaseDropdown>
    ...
    <router-link
      v-if="userStore.hasAbilities(abilities.EDIT_INVOICE) && canEditInvoice(row)"
    ...
    <BaseDropdownItem
      v-if="canCreateRectificative(row)"
      @click="createRectificative(row)"
    >
      <BaseIcon
    ...
    <BaseDropdownItem
      v-if="userStore.hasAbilities(abilities.DELETE_INVOICE) && canDeleteInvoice(row)"
...
function isFiscallyLocked(row) {
  return invoiceStore.isFiscallyLocked(row)
}

function canEditInvoice(row) {
  return row.allow_edit && !isFiscallyLocked(row)
}

function canDeleteInvoice(row) {
  return !isFiscallyLocked(row)
}

function canCreateRectificative(row) {
  return (
    userStore.hasAbilities(abilities.CREATE_INVOICE) &&
    isFiscallyLocked(row) &&
    row.invoice_kind !== 'RECTIFICATIVE'
  )
}
...
async function createRectificative(invoice) {
  dialogStore
    .openDialog({
      title: t('general.are_you_sure'),
    ...
    .then((res) => {
      if (res) {
        invoiceStore.createRectificativeInvoice({ id: invoice.id }).then((response) => {
...
async function onMarkAsSent(id) {
  dialogStore
    ...
    .then((response) => {
      ...
      if (response) {
        invoiceStore.markAsSent(data).then(() => {
          props.table && props.table.refresh()
        })
      }
    })
