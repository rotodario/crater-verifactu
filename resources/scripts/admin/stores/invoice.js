...
export const useInvoiceStore = (useWindow = false) => {
  ...
  return defineStoreFunc({
    ...
    actions: {
      isFiscallyLocked(invoice) {
        return Boolean(
          invoice &&
            (invoice.fiscal_locked_at ||
              ['ISSUED', 'SUBMITTED', 'ACCEPTED'].includes(invoice.fiscal_status))
        )
      },
      ...
      createRectificativeInvoice(data) {
        return new Promise((resolve, reject) => {
          axios
            .post(`/api/v1/invoices/${data.id}/rectify`, data)
            .then((response) => {
              notificationStore.showNotification({
                type: 'success',
                message: 'Rectificative invoice created successfully.',
              })
              resolve(response)
            })
            .catch((err) => {
      ...
      getNextNumber(params, setState = false) {
        return new Promise((resolve, reject) => {
          ...
          axios
            .get(`/api/v1/next-number?key=${key}`, { params })
            .then((response) => {
              if (setState) {
      ...
      selectAllInvoices() {
        if (this.selectedInvoices.length === this.invoices.length) {
          ...
          let allInvoiceIds = this.invoices
            .filter((invoice) => !this.isFiscallyLocked(invoice))
