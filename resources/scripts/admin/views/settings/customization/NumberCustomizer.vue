<template>
  ...
  <div class="overflow-x-auto">
    <table class="w-full mt-6 table-fixed">
      ...
      <thead>
        <tr>
          <th class="px-5 py-3 text-sm not-italic font-medium leading-5 text-left text-gray-700 border-t border-b border-gray-200 border-solid"></th>
      ...
      >
        <template #item="{ element }">
          <tr class="relative">
            ...
            <td class="px-5 py-4">
              <label class="block text-sm not-italic font-medium text-primary-800 whitespace-nowrap mr-2 min-w-[200px]">
                {{ element.label }}
              </label>
            ...
            <td class="px-5 py-4 text-left align-middle">
              <BaseInputGroup :label="element.paramLabel" class="lg:col-span-3" required>
                <BaseInput
            ...
            <td class="px-5 py-4 text-right align-middle pt-10">
              <BaseButton variant="white" @click.prevent="removeComponent(element)">
                Remove
                <template #left="slotProps">
                  <BaseIcon name="XIcon" class="!sm:m-0" :class="slotProps.class" />
                </template>
              </BaseButton>
            </td>
          </tr>
        ...
        <template #footer>
          <tr>
            <td colspan="2" class="px-5 py-4">
              <BaseInputGroup :label="previewLabel">
                <BaseInput v-model="nextNumber" disabled :loading="isFetchingNextNumber" />
              </BaseInputGroup>
            </td>
...
const resolvedSettingKey = computed(() => {
  return props.settingKey || `${props.type}_number_format`
})

const resolvedNextNumberKey = computed(() => {
  return props.nextNumberKey || props.type
})

const title = computed(() => {
  return props.titleKey
    ? t(props.titleKey)
    : t(`settings.customization.${props.type}s.${props.type}_number_format`)
})

const description = computed(() => {
  return props.descriptionKey
    ? t(props.descriptionKey)
    : t(`settings.customization.${props.type}s.${props.type}_number_format_description`)
})

const previewLabel = computed(() => {
  return props.previewKey
    ? t(props.previewKey)
    : t(`settings.customization.${props.type}s.preview_${props.type}_number`)
})

const successMessage = computed(() => {
  return props.successMessageKey || `settings.customization.${props.type}s.${props.type}_settings_updated`
})
...
async function setInitialFields() {
  let data = {
    format: companyStore.selectedCompanySettings[resolvedSettingKey.value],
  }
...
async function getNextNumber() {
  ...
  let data = {
    key: resolvedNextNumberKey.value,
...
async function submitForm() {
  ...
  let data = { settings: {} }
  data.settings[resolvedSettingKey.value] = getNumberFormat.value
