      ...
      >
        ...
        <template #cell-checkbox="{ row }">
          <div class="relative block">
            <BaseCheckbox
              ...
              v-model="selectField"
              :value="row.data.id"
              :disabled="isFiscallyLocked(row.data)"
            />
          </div>
        </template>
        ...
        <template #cell-status="{ row }">
          <div class="flex flex-col items-start gap-1">
            ...
            <BaseBadge
              :bg-color="getFiscalBadgeColor(row.data.fiscal_status).bgColor"
              :color="getFiscalBadgeColor(row.data.fiscal_status).color"
            >
              Fiscal {{ row.data.fiscal_status }}
            </BaseBadge>
            <BaseBadge
              :bg-color="getKindBadgeColor(row.data.invoice_kind).bgColor"
              :color="getKindBadgeColor(row.data.invoice_kind).color"
            >
              {{ getKindLabel(row.data.invoice_kind) }}
            </BaseBadge>
          </div>
        </template>
...
function isFiscallyLocked(invoice) {
  return invoiceStore.isFiscallyLocked(invoice)
}
