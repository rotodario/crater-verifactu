...
const canCreateRectificative = computed(() => {
  return (
    invoiceData.value &&
...
function getKindBadgeColor(kind) {
  if (kind === 'RECTIFICATIVE') {
    return { bgColor: '#E9D8FD', color: '#553C9A' }
  }
...
async function onCreateRectificative() {
  dialogStore
    .openDialog({
      title: t('general.are_you_sure'),
      message: 'Se creara una factura rectificativa en borrador basada en esta factura.',
      yesLabel: t('general.ok'),
      noLabel: t('general.cancel'),
      variant: 'primary',
      hideNoButton: false,
    ...
    .then(async (response) => {
      if (response) {
        const result = await invoiceStore.createRectificativeInvoice({
          id: invoiceData.value.id,
        })
        router.push(`/admin/invoices/${result.data.data.id}/edit`)
      }
    })
}
    ...
    <BasePageHeader :title="pageTitle">
      <template #actions>
        ...
        <div v-if="invoiceData" class="ml-3 flex items-center gap-2">
          <BaseBadge
            :bg-color="getFiscalBadgeColor(invoiceData.fiscal_status).bgColor"
            :color="getFiscalBadgeColor(invoiceData.fiscal_status).color"
          >
            Fiscal {{ invoiceData.fiscal_status }}
          </BaseBadge>
          <BaseBadge
            :bg-color="getKindBadgeColor(invoiceData.invoice_kind).bgColor"
            :color="getKindBadgeColor(invoiceData.invoice_kind).color"
          >
            {{ getKindLabel(invoiceData.invoice_kind) }}
          </BaseBadge>
        </div>

        <BaseButton
          v-if="canCreateRectificative"
          variant="primary-outline"
          class="ml-3 text-sm"
          @click="onCreateRectificative"
        >
          Crear rectificativa
        </BaseButton>
