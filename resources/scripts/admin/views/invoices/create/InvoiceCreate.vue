    ...
    <form @submit.prevent="submitForm">
      <BasePageHeader :title="pageTitle">
        ...
        <template #actions>
          ...
          <BaseButton
            :loading="isSaving"
            :disabled="isSaving || (isEdit && invoiceStore.newInvoice.allow_edit === false)"
            variant="primary"
            type="submit"
          >
...
watch(
  () => invoiceStore.newInvoice.allow_edit,
  (allowEdit) => {
    if (isEdit.value && allowEdit === false) {
      notificationStore.showNotification({
        type: 'error',
        message: 'Fiscally issued invoices cannot be edited.',
      })
      router.replace(`/admin/invoices/${route.params.id}/view`)
    }
  }
)
